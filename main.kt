...
class BuddySystem(n: Int) {
    ...
    private fun dividir(bloque: Bloque, n: Int) : Bloque {
        ...
        if (bloqueA.orden() > n) {
            var hijoDer : Bloque = Bloque()
            var hijoIzq : Bloque = Bloque()
    ...
    fun liberar(nombre: String) : Boolean {
        var i : Int  = procesosNombres.indexOf(nombre)
        var e : Boolean = false
        if (i != -1) {
            e = true
            var orden : Int = procesosOrden.get(i)
...
fun main(args: Array<String>) {
    var particiones : LinkedList<BuddySystem> = LinkedList()
    val lector = Scanner(System.`in`)
    var linea : String
    var palabras : List<String> 
    var j : Int = args[0].toInt()
    var bin : String = j.toString(2)
    j = 0
            ...
            "MOSTRAR" -> {
                ...
                for (k in particiones) {
                    for (i in 0 until k.procesosNombres.size) {
                        var espacio : Int = (2.0.pow(k.procesosOrden[i])).toInt()
